...
class Test extends FunSuite with CatchErrorSuite {
  ...
  test("cons tests") {
    assertResult(ListT(NumT())) {
      typeOf("(cons 1 (cons 2 (nil : Num)))")
    }
    assertResult(ListT(NumT())) {
      typeOf("(cons 1 (nil : Num))")
    }
     assertResult(ListT(BoolT())) {
      typeOf("(cons true (cons false (nil : Bool)))")
    }
  }
  
  test("cons test exceptions") {
    intercept[TypeException] {
      typeOf("(cons 1 2)")
    }
    intercept[TypeException] {
      typeOf("(cons 1 (cons 2 (nil : Bool)))")
    }
    intercept[TypeException] {
      typeOf("(cons (cons 2 (nil : Bool)) 1)")
    }
    intercept[TypeException] {
      typeOf("(cons (cons 1 (nil : Num)) (nil : Num))")
    }
  }
  
    test("incorrect type check - cons with boolean nil") {
    intercept[TypeException] {
      typeOf("(cons 4 (cons 5 (cons 6 (nil : Bool))))")
    }
  }
  
   test("list with list") {
     intercept[TypeException] {
      typeOf("(list : Num((list : Num (2))))")
    }
  }
  
    test("pet scope") {
      intercept[TypeException] {
      typeOf("(letrec (((x : Num) 1) ((y : Bool) (set x true))) (or x y))")
    }
  }
  
  test("let-rec #1") {
    assertResult(NumT()) {
      typeOf("(letrec (((x : Num) x)) x)")
    }
  }
  
  test("rec-lam") {
   assertResult(FunT(List(NumT()), BoolT())) {
      typeOf("(rec-lam (f : Num -> Bool) (x) (f x))")
    }
  }
